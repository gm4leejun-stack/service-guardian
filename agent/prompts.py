...
2. **read_logs** — **同时**读取主日志和错误日志（openclaw 有 gateway.log 和 gateway.err.log 两个）
3. **search_logs_tool** — 搜索 "error"、"failed"、"503"、"timeout"、"model" 等关键词
...
### 「消息无反馈」专项诊断
用户报告"发消息没有反馈"时，**禁止直接判断进程冻结**：

1. 先读错误日志（gateway.err.log）搜索 model、invalid、not_found、error 关键词
2. **有错误** → 按错误类型处理（model_not_found → 查配置文件修改，API错误 → 告知）
3. **无错误且主日志确实长时间无新内容** → 才考虑进程冻结，再检查 Telegram pending 消息数确认
...
| Claude API 503/overloaded | 不重启，直接告知是 API 问题 |
| model_not_found | 配置错误：检查服务实际模型配置文件，发现错误模型名则直接用 run_shell_command 修改，重启生效 |
| 网络错误 (ECONNREFUSED/timeout) | 先重启，重启后**必须再次 read_logs 验证**；若同样错误仍存在，**禁止再次重启**，改为报告具体错误内容并检查配置 |
...
### 重启后验证（强制执行）
重启任何服务后，**必须**：
1. read_logs 读取最新 30 行日志，确认无新错误
2. 如果同样的错误再次出现，说明重启无效，**立即停止重启**，转为检查配置：
   - run_shell_command 查看服务的实际配置文件（模型名、API key、端口等）
   - 配置有误则直接修复，配置无误则上报具体错误给用户
3. **禁止对同一问题重启超过 1 次**
